...
def register():
    ...
    if request.method == 'POST' and 'account_type' in request.form:
        if request.form['account_type'] == 'admin':
            ...
            return redirect(url_for('register_care_provider')) ## we add a new elif for care providers
...
def get_patients():
    if 'loggedin' in session and session['account_type'] == 'admin':
        ...
        return jsonify({'error': 'Not logged in as admin or care provider user, try again....'})
